...
export async function up(knex: Knex): Promise<void> {
  return knex.raw(`
   CREATE OR REPLACE FUNCTION create_trans_out(
    ...
    RETURNS boolean -- The function will return true or false
    LANGUAGE plpgsql
    AS $$
    DECLARE
        -- This variable will hold the start_date truncated to the beginning of the day (00:00:00)
        target_date timestamptz;
    BEGIN
        target_date := date_trunc('day', start_date);
        target_date := target_date + (days * '1 day'::interval);
...
export async function down(knex: Knex): Promise<void> {
  return knex.raw(`
      DROP FUNCTION create_trans_out;
      DROP FUNCTION is_past_days;
    `);
}
